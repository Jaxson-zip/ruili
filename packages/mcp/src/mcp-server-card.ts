 ...
 */
export function buildMcpServerCard(appVersion: string, appUrl = "https://ruili.resume") {
	...
	const tools = [
		...
		},
		{
			...
			description: [
				...
				"Analyses are created from the 锐历 web app AI flow, not from MCP.",
		...
		},
		{
			...
			title: "Import Resume",
			description: [
				"Create a new resume from a full ResumeData JSON object (e.g. an exported file from 锐历).",
	...
	return {
		...
		serverInfo: {
			...
			icons: [
				{ src: new URL("/icon/light.svg", appOrigin).toString(), mimeType: "image/svg+xml", theme: "light" as const },
				{ src: new URL("/icon/dark.svg", appOrigin).toString(), mimeType: "image/svg+xml", theme: "dark" as const },
			],
		},
		tools,
		prompts,
