...
describe("buildMcpServerCard", () => {
	const card = buildMcpServerCard("1.2.3", "https://app.example.com");
	...
	it("identifies the server as reactive-resume", () => {
		...
		expect(card.serverInfo.title).toBe("锐历");
		expect(card.serverInfo.websiteUrl).toBe("https://app.example.com");
	});
