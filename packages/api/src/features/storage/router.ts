...
import { getStorageService, processImageForUpload, uploadFile, verifyImageUpload } from "./service";
...
export const storageRouter = {
	uploadFile: protectedProcedure
		...
		.handler(async ({ context, input: file }) => {
			...
			try {
				verifyImageUpload({ declaredMimeType: file.type, data: new Uint8Array(fileBuffer) });
			} catch {
