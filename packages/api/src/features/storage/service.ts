...
export function verifyImageUpload(input: { declaredMimeType: string; data: Uint8Array }): string {
	if (!isImageFile(input.declaredMimeType)) throw new Error("UNSUPPORTED_IMAGE_MIME_TYPE");

	const detectedMimeType = detectImageMimeType(input.data);
	if (!detectedMimeType) throw new Error("IMAGE_SIGNATURE_INVALID");
	if (detectedMimeType !== input.declaredMimeType) throw new Error("IMAGE_MIME_MISMATCH");
