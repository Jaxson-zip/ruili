...
const wordFileInputSchema = createFileInputSchema(["doc", "docx"]);
const imageFileInputSchema = createFileInputSchema(["gif", "jpeg", "jpg", "png", "webp"]);
const cloudOcrCredentialsSchema = z.object({
...
function isCloudOcrConfigurationError(error: unknown): boolean {
	return (
		error instanceof Error &&
		(error.message === "CLOUD_OCR_NOT_CONFIGURED" || error.message === "CLOUD_OCR_ENDPOINT_NOT_ALLOWED")
	);
}

function isCloudOcrGatewayError(error: unknown): boolean {
	return error instanceof Error && error.message.startsWith("CLOUD_OCR_");
}
...
function throwCloudOcrConfigError(): never {
	throw new ORPCError("BAD_REQUEST", { message: "Cloud OCR is not configured or the endpoint is not allowed." });
}

function throwCloudOcrGatewayError(): never {
	throw new ORPCError("BAD_GATEWAY", { message: "Could not read the file with cloud OCR." });
}
...
export const aiRouter = {
	parsePdf: protectedProcedure
		...
		.input(
			z.object({
				...
				ocr: cloudOcrCredentialsSchema.optional(),
			}),
		)
	...
	parseImage: protectedProcedure
		.route({
			...
			path: "/ai/parse-image",
			tags: ["AI"],
			operationId: "parseResumeImage",
			summary: "Parse an image file into resume data",
			description:
				"Extracts text from a resume screenshot/image using configured cloud OCR, then converts it into structured resume data with the selected AI provider. Requires authentication, AI credentials, and cloud OCR configuration.",
			successDescription: "The image was successfully parsed into structured resume data.",
		})
		.input(
			z.object({
				aiProviderId: z.string().optional(),
		...
		.use(aiRequestRateLimit)
		.errors({
			BAD_GATEWAY: { message: "The OCR or AI provider returned an error or is unreachable.", status: 502 },
			BAD_REQUEST: { message: "Cloud OCR is not configured or the AI returned invalid data.", status: 400 },
		})
		.handler(async ({ context, input }): Promise<ResumeData> => {
			try {
				const provider = await getRunnableProvider(context.user.id, input.aiProviderId);
				return await aiService.parseImage({
					provider: provider.provider,
					model: provider.model,
					apiKey: provider.apiKey,
					baseURL: provider.baseURL ?? "",
				...
				if (isCredentialEncryptionUnavailable(error)) throwCredentialEncryptionUnavailable();
				if (isCloudOcrConfigurationError(error)) throwCloudOcrConfigError();
				if (isCloudOcrGatewayError(error)) throwCloudOcrGatewayError();
				if (isInvalidAiBaseUrlError(error)) throwAiProviderConfigError();
				if (isAiProviderGatewayError(error)) throwAiProviderGatewayError();
				if (error instanceof ZodError) throwResumeStructureError(error);
				throw error;
