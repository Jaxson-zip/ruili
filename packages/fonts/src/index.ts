...
const localIbmPlexSerifRegular = new URL("./files/ibm-plex-serif-400.ttf", import.meta.url).toString();
const localIbmPlexSerifSemiBold = new URL("./files/ibm-plex-serif-600.ttf", import.meta.url).toString();
const localNotoSansCjkRegular = new URL("./files/noto-sans-cjk-sc-regular.otf", import.meta.url).toString();
const localNotoSansCjkBold = new URL("./files/noto-sans-cjk-sc-bold.otf", import.meta.url).toString();
const localNotoSerifCjkRegular = new URL("./files/noto-serif-cjk-sc-regular.otf", import.meta.url).toString();
const localNotoSerifCjkBold = new URL("./files/noto-serif-cjk-sc-bold.otf", import.meta.url).toString();

const localFontSourceWeights = ["100", "200", "300", "400", "500", "600", "700", "800", "900"] satisfies FontWeight[];

function buildLocalFontSources(regularSource: string, boldSource: string) {
	const sources: Partial<Record<FontFileWeight, string>> = {};

	for (const weight of localFontSourceWeights) {
		const source = Number(weight) >= 600 ? boldSource : regularSource;
		sources[weight] = source;
		sources[`${weight}italic` as FontFileWeight] = source;
	}

	return sources;
}

const localFontSources: Partial<Record<string, Partial<Record<FontFileWeight, string>>>> = {
	"IBM Plex Serif": buildLocalFontSources(localIbmPlexSerifRegular, localIbmPlexSerifSemiBold),
	"Noto Sans SC": buildLocalFontSources(localNotoSansCjkRegular, localNotoSansCjkBold),
	"Noto Serif SC": buildLocalFontSources(localNotoSerifCjkRegular, localNotoSerifCjkBold),
};
...
export function getFontSearchKeywords(family: string) {
	...
	return unique(
		[family, displayName, chinesePrioritySet.has(family) ? "中文" : undefined].filter((keyword): keyword is string =>
			Boolean(keyword),
		),
	);
}
...
export function getWebFontSource(family: string, weight: FontWeight = "400", italic = false) {
	...
	const localSource = localFontSources[family]?.[key] ?? (italic ? localFontSources[family]?.[weight] : undefined);
