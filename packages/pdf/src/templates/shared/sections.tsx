...
const ProfileSection = ({ sectionId = "profiles", sectionData }: ItemSectionProps<ProfileItem> = {}) => {
	...
	const placement = useTemplatePlacement();
	const compactChinese = true;

	if (items.length === 0) return null;

	if (compactChinese) {
		const columns = placement === "sidebar" ? 1 : Math.min(2, Math.max(1, profiles.columns));

		return (
			<SectionShell sectionId={sectionId} title={profiles.title}>
				<SectionItems columns={columns}>
					{items.map((item) => {
...
const SkillsSection = ({ sectionId = "skills", sectionData }: ItemSectionProps<SkillItem> = {}) => {
	...
	if (compactChinese) {
		const columns = placement === "sidebar" ? 1 : Math.min(2, Math.max(1, skills.columns));
			...
			<SectionShell sectionId={sectionId} title={skills.title}>
				<SectionItems columns={columns}>
					{items.map((item) => {
						...
						return (
							<SectionItem key={item.id} style={{ rowGap: metrics.gapY(0.1) }}>
								<Text>
									<Bold>{item.name}</Bold>
									{details ? `：${details}` : ""}
								</Text>
...
const LanguagesSection = ({ sectionId = "languages", sectionData }: ItemSectionProps<LanguageItem> = {}) => {
	...
	if (compactChinese) {
		const columns = placement === "sidebar" ? 1 : Math.min(2, Math.max(1, languages.columns));
