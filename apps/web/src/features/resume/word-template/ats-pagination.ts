export type AtsMeasuredItemRect = {
	bottom: number;
	top: number;
};

export type AtsSectionMeasurement = {
	footerGap?: number;
	items: AtsMeasuredItemRect[];
};

export type AtsMeasuredPageBlock<TSectionId extends string> = {
	end: number;
	sectionId: TSectionId;
	start: number;
};

export type AtsPaginationInput<TSectionId extends string> = {
	continuationPageCapacity: number;
	firstPageCapacity: number;
	minSectionStartItems?: Partial<Record<TSectionId, number>>;
	sectionMeasurements: Partial<Record<TSectionId, AtsSectionMeasurement>>;
	sectionOrder: readonly TSectionId[];
};

const EPSILON = 0.001;

export function paginateAtsMeasuredLayout<TSectionId extends string>({
	continuationPageCapacity,
	firstPageCapacity,
	minSectionStartItems,
	sectionMeasurements,
	sectionOrder,
}: AtsPaginationInput<TSectionId>) {
	const pages: AtsMeasuredPageBlock<TSectionId>[][] = [[]];
	...
	for (const sectionId of sectionOrder) {
		...
		while (start < items.length) {
			...
			pages[pageIndex].push({ end, sectionId, start });
...
function countMeasuredItemsThatFit(
	items: readonly AtsMeasuredItemRect[],
	start: number,
	remaining: number,
	footerGap: number,
) {
