...
const domesticProviderPresets: ProviderPreset[] = [
	{
		label: "DeepSeek",
...
function statusBadge(provider: SavedProvider) {
	if (provider.testStatus === "success") {
		return <Badge className="bg-emerald-600 text-white">已测试</Badge>;
	}
		...
		<div className="grid gap-4 rounded-md border bg-card p-4 md:grid-cols-[1fr_auto]">
			<div className="min-w-0 space-y-2">
				<div className="flex flex-wrap items-center gap-2">
					...
					{provider.enabled ? <Badge variant="outline">启用</Badge> : null}
				</div>
	...
	return (
		<div className="rounded-md border bg-card p-4">
			...
			<div className="mb-4 flex flex-wrap items-center gap-2">
				<span className="text-muted-foreground text-sm">常用国内模型</span>
				{domesticProviderPresets.map((preset) => (
